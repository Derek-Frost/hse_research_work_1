...
def load_local_safetensors(model, param_file: str):
    """
    Пример функции, загружающей веса из локального safetensors-файла
    в модельный объект. Предполагается, что атрибуты параметров модели
    совпадают с ключами в safetensors.
    """
    if not os.path.exists(param_file):
        raise FileNotFoundError(f"Файл параметров {param_file} не найден.")

    # данные из safetensors
    weights = load_file(param_file)
    for name, tensor in weights.items():
        # есть ли в нашем объекте model параметр с таким же именем
        if hasattr(model, name):
            param = model.get_parameter(name)
            param.copy_(tensor.to(param.dtype).to(param.device))
        else:
            # можно логировать пропущенные веса
            pass
...
def compose_new_params(
    ...
    mm = policy.get_mask(learnable_params[param_name]).to(S.dtype)
    return (
        U @ torch.diag_embed(S * mm) @ V.T
    ) * (S.sum() / (S * mm).sum())
